...
import { LogOut, Star, AlertCircle, CheckCircle, Upload, X, Sparkles } from 'lucide-react';
...
export default function AdminFeatured() {
  ...
  const handleLogoChange = (e) => {
    ...
    if (file) {
      // Increased size limit to 10MB for high-quality images and SVG
      ...
      if (!allowedTypes.includes(file.type) && !file.type.startsWith('image/')) {
        setMessage({ type: 'error', text: 'Please select a valid image file (PNG, JPG, JPEG, SVG, GIF, WEBP, etc.)' });
        return;
      }
  ...
  // Handle create featured job
  const handleCreateFeaturedJob = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setMessage({ type: '', text: '' });
    ...
    try {
      ...
      // Add logo file ONLY if it exists and is a valid File object
      if (logoFile && logoFile instanceof File && logoFile.size > 0) {
        console.log('Adding logo file:', logoFile.name, logoFile.type, logoFile.size);
        formData.append('logo', logoFile);
      } else {
        console.log('No logo file to upload');
      }

      // Post to featured endpoint
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to create featured job');
      }

      setMessage({ type: 'success', text: '⭐ Featured job posted successfully!' });
  ...
  if (currentPage === 'login') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-500 via-yellow-500 to-orange-600 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-lg shadow-2xl p-8">
          ...
          <h1 className="text-3xl font-bold text-center text-gray-800 mb-2">Featured Jobs Admin</h1>
          <p className="text-center text-gray-600 mb-8">Post premium job listings</p>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white py-2 rounded-lg font-medium hover:from-amber-600 hover:to-orange-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      <div className="bg-white shadow-lg border-b-4 border-amber-400">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Featured Jobs Admin</h1>
              <p className="text-sm text-amber-600">Premium Job Listings</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-xl border-2 border-amber-200 p-8">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-gray-800">Post a Featured Job</h2>
            </div>
            <button
              onClick={() => setShowJsonUpload(!showJsonUpload)}
              className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 py-2 rounded-lg hover:from-green-600 hover:to-emerald-700 transition"
            >
          ...
          {/* JSON Upload Section */}
          {showJsonUpload && (
            <div className="mb-6 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-lg">
...
{`{
  ...
  "tags": ["Featured", "Engineering", "Senior"],
          ...
          <div className="space-y-6">
            ...
            <div className="border-b pb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  {!logoPreview ? (
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-amber-300 rounded-lg cursor-pointer hover:border-amber-500 bg-amber-50 transition">
                      <Upload className="w-8 h-8 text-amber-400 mb-2" />
                      <span className="text-sm text-gray-600">Click to upload logo</span>
                      <span className="text-xs text-gray-400 mt-1">PNG, JPG, JPEG, SVG, GIF, WEBP (up to 10MB)</span>
                      <input
                        type="file"
                        accept="image/*"
                    ...
                    <div className="relative inline-block">
                      <img
                        src={logoPreview}
                        alt="Logo preview"
                        className="h-32 w-32 object-contain border-2 border-amber-300 rounded-lg bg-white"
                      />
                      <button
                        type="button"
                        onClick={removeLogo}
            ...
            <div className="border-b pb-6">
              ...
              <textarea
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none mb-4"
                rows="3"
              />
              <textarea
            ...
            <div className="pb-6">
              ...
              <div className="bg-amber-50 border-2 border-amber-200 rounded-lg p-4 flex items-center gap-3">
                ...
                <p className="text-sm text-gray-700">
                  <strong className="text-amber-700">Featured Job:</strong> This listing will be automatically marked as featured and displayed prominently on the job board.
                </p>
              </div>
            </div>

            <button
              onClick={handleCreateFeaturedJob}
              disabled={isLoading || !jobForm.title || !jobForm.company || !jobForm.category || !jobForm.location || !jobForm.department}
              className="w-full bg-gradient-to-r from-amber-500 via-yellow-500 to-orange-500 text-white py-3 rounded-lg font-semibold hover:from-amber-600 hover:via-yellow-600 hover:to-orange-600 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-lg flex items-center justify-center gap-2"
            >
              {isLoading ? (
                'Posting Featured Job...'
              ) : (
                <>
